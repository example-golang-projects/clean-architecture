...
func WriteError(ctx context.Context, w http.ResponseWriter, err error) {
	errIn := errorpkg.ToErrorInterface(err)
	jsonErr := errorpkg.ToErrorJSON(errIn)
	...
	w.Header().Set("Content-Type", "internal/json") // Error responses are always JSON
...
func WriteReponse(ctx context.Context, w http.ResponseWriter, status int, payload interface{}) {
	w.Header().Set("Content-Type", "internal/json; charset=utf-8")
